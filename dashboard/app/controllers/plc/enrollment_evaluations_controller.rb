class Plc::EnrollmentEvaluationsController < ApplicationController
  ...
  def assert_unit_assignment_state
    ...
    #There's a longer discussion about when exactly can someone retake the evaluation or redo the selections. For now
    #we'll just say that you can no longer redo the assessment once you've done an assignment

    if plc_unit_assignment.plc_task_assignments.where.not(status: Plc::EnrollmentTaskAssignment::NOT_STARTED).pluck(:status).any?
      raise 'Cannot do evaluation actions when there are task assignments in progress / completed'
    end
  end
end
