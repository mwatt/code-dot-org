...
var FacilitatorListFormPart = React.createClass({
  ...
  validate: function (shouldValidate = true) {
    ...
    if (shouldValidate) {
      // Allow one empty row.
      // TODO: upgrade UI to select from list of known facilitators
      if (this.props.facilitators.length > 1 ||
        this.props.facilitators[0].name || this.props.facilitators[0].email) {
  ...
  handleFacilitatorChange: function (facilitator) {
    alert(facilitator.email);
  },

  render: function () {
    var facilitatorOptions = _.difference(
      this.props.availableFacilitators,
